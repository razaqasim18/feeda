...
class ProductRepository extends Repository
{
 ...
 public static function getProductsByBrand($brandId)
    {
        return self::query() // Use self:: instead of $this->
    ...
    public static function storeByRequest(ProductRequest $request): Product
    {
        ...
        if ($request->is('api/*')) {
            ...
            foreach ($request->color ?? [] as $color) {
                ...
                // Assuming $request is your form request and $colorId is the current color ID
                if ($request->hasFile("color.{$color['id']}.cimage")) {
                    $image = $request->file("color.{$color['id']}.cimage");

                    // Store the image (adjust path as needed)
                    $imagePath = $image->store('color_images');

                    // Attach color with both price and image path
                    ...
                    // Attach without image if not provided
                    $product->colors()->attach($color['id'], [
                        'price' => $color['price'],
                        'code' =>  $color['code'],
                        'quantity' =>  $color['quantity']
