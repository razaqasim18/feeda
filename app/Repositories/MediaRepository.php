...
class MediaRepository extends Repository
{
    ...
    public static function videoStoreByRequest(UploadedFile $file, string $path, ?string $type = null): Media
    {
        $path = Storage::disk('public')->put('/' . trim($path, '/'), $file);

        $extension = $file->extension();
        if (! $type) {
            $type = in_array($extension, ['mp4', 'avi', 'mpeg', 'quicktime', 'x-matroska']) ? 'video' : $extension;
        }
