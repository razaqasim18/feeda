...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $brandsURL= BrandRepository::model()::where('brands.is_active', 1)
        //     ->leftJoin('media', 'brands.media_id', '=', 'media.id')
        //     ->select(
        //         'brands.id',
        //         'brands.name',
        //         'brands.shop_id',
        //         'media.src as image_url'  // Get the image URL directly
        //     )
        //     ->orderBy('brands.name')
        //     ->get()
        //     ->toArray();
        ...
        $discountProducts = ProductRepository::query()->isActive()
            ->when($shop, function ($query) use ($shop) {
                return $query->where('shop_id', $shop->id);
            })->withCount('orders as orders_count')
            ->where("discount_price","!=",'0.00')
            ->withAvg('reviews as average_rating', 'rating')
