<template>
    <div>
        ...
        <button v-if="!isProcessing" class="px-6 py-4 w-full mt-4 bg-primary rounded-[10px] text-white text-base font-medium" @click="processOrderConfirm">
            {{ $t('Place Order') }}
        </button>
        <button v-else class="px-6 py-4 w-full mt-4 bg-primary-200 rounded-[10px] text-primary text-base font-semibold flex items-center justify-center gap-2" disabled>
...
let isbirthdayCoupon = ref(0);
 console.log("initial "+isbirthdayCoupon);
const coupon = ref("");
    ...
    axios.post("/cart/checkout", {
        ...
        if (hasCoupon.value && coupon.value.length > 0) {
            ...
            basketStore.coupon_code = coupon.value;
             console.log("before "+isbirthdayCoupon);
            isbirthdayCoupon = response.data.data.checkout.is_birthday_coupon;
            console.log("after "+isbirthdayCoupon);
