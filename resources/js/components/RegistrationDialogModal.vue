                        ...
                        <TransitionChild as="template" enter="ease-out duration-300"
                            ...
                            <DialogPanel
                                ...
                                <div class="bg-white p-5 sm:p-8 relative" :class="master.langDirection==='rtl' ? 'text-right' : 'text-left'">
                                    ...
                                    <div class="mt-4">
                                        <label for="date" class="text-slate-700 text-base font-normal leading-normal mb-2 block">{{ $t('Date of Birth') }}</label>
                                        <input type="date" id="date" v-model="registerFormData.date_of_birth" class="text-base font-normal w-full p-3 placeholder:text-slate-400 border focus:border-primary outline-none"
                                            :class="(registerErrors?.date_of_birth ? 'border-red-500' : 'border-slate-200') + (master.langDirection==='rtl' ? ' rounded-tl-lg rounded-bl-lg' : ' rounded-tr-lg rounded-br-lg')">
                                        <span v-if="registerErrors && registerErrors?.date_of_birth" class="text-red-500 text-sm">
                                            {{ registerErrors?.date_of_birth[0] }}
                                        </span>
                                    </div>
...
const registerFormSubmit = () => {
    ...
    axios.post('/registration', registerFormData.value).then((response) => {
        ...
        registerFormData.value.date_of_birth = null;
