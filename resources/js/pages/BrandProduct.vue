<template>
    <div>
        ...
        <div class="main-container py-12">
            ...
            <div v-if="isLoadingMore" class="flex justify-center items-center w-full mt-8">
                 <SkeletonLoader class="w-full h-[220px] sm:h-[330px]" />
            </div>
            
            <!-- End of results message -->
            <!-- <div v-if="hasReachedEnd && products.length > 0" class="flex justify-center items-center w-full mt-8">
                <div class="text-slate-800 text-base font-normal leading-normal">
                    {{ $t("You've reached the end of results") }}
                </div>
            </div> -->
        </div>
...
watch(() => route.params.slug, () => {
    currentPage.value = 1;
    filterFormData.value.brand_id = route.params.slug;
    fetchProducts();
});
...
const isScrolling = ref(false); // Add this for debouncing
...
const fetchProducts = async () => {
    ...
    const loading = currentPage.value === 1 ? isLoading : isLoadingMore;
    loading.value = true;
    ...
    try {
        ...
        // Update filters and price range
        ...
        // Update end detection
        hasReachedEnd.value = response.data.data.products.length < perPage;
...
// Modify your clearFilter and applyFilter to reset scroll state
const clearFilter = () => {
    filterFormData.value = {
        rating: null,
        short_by: "default",
