            ...
            <form @submit.prevent="submitForm()">
                <div class="max-w-[920px] mx-auto bg-white rounded-xl md:rounded-2xl p-3 md:p-4 xl:p-6">
                    ...
                    <div class="p-3 bg-white rounded-xl border border-slate-100 mt-3">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <v-select :options="countries" label="name" :reduce="country => country.name"
                                    v-model="formData.country" :placeholder="$t('Select Country')"  :dir="masterStore.langDirection || 'ltr'" />
                            ...
                            <div>
                                ...
                                <div class="flex">
                                    <span class="text-slate-700 text-base font-normal leading-normal bg-slate-100 p-2 border-y flex items-center" :class="masterStore.langDirection == 'rtl' ? 'border-r rounded-tr-lg rounded-br-lg' : 'border-l rounded-tl-lg rounded-bl-lg'">
                                        +{{ formData.phone_code || '00' }}
                                    </span>
                                    <input type="text" v-model="formData.phone" :placeholder="$t('Enter phone number')" class="text-base font-normal w-full p-3 placeholder:text-slate-400 border focus:border-primary outline-none" :class="[errors?.phone ? 'border-red-500' : 'border-slate-200', masterStore.langDirection == 'rtl' ? 'rounded-tl-lg rounded-bl-lg' : 'rounded-tr-lg rounded-br-lg']" :maxlength="masterStore.phoneMaxLength" @input="formData.phone = formData.phone.replace(/[^\d]/g, '')">
                                </div>
                        ...
                        <button v-if="!isLoading" type="submit" class="bg-primary px-4 md:px-6 py-2 md:py-4 rounded-lg :md:rounded-[10px] text-white mt-6">
                            {{ $t('Update Profile') }}
                        </button>
                        <button v-else type="button" class="bg-primary-200 px-4 md:px-6 py-2 md:py-4 rounded-lg :md:rounded-[10px] text-primary mt-6 flex items-center justify-center cursor-not-allowed gap-2" disabled>
